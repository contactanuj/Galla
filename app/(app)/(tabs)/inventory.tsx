...
function ProductCard({ product, onPress }: { product: ProductWithCategory; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity onPress={onPress} className="bg-white dark:bg-slate-900 rounded-xl p-3 mb-3 border border-slate-200 dark:border-slate-700 shadow-sm active:bg-slate-50 flex-row items-center" accessibilityLabel={product.name}>
      ...
      <View className="flex-1">
        ...
        <View className="flex-row items-center gap-2">
          ...
          {hasVariants && <Text className="text-xs text-primary-600 ml-2">· {product.variant_count} variants</Text>}
        </View>
      </View>
    ...
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      <View className="px-4 pt-4 pb-2 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
        ...
        <View className="flex-row items-center gap-2">
          ...
          <TouchableOpacity onPress={listening ? stopListening : startListening} className={`rounded-lg px-3 py-2.5 ${listening ? 'bg-red-50' : 'bg-slate-100 dark:bg-slate-800'}`} accessibilityLabel={t('common.voice')}>
