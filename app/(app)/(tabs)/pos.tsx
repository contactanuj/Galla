...
function ProductCard({ product, onAdd }: { product: ProductWithCategory; onAdd: () => void }) {
  ...
  return (
    <TouchableOpacity onPress={onAdd} className="bg-white dark:bg-slate-900 rounded-xl p-4 mb-2 border border-slate-200 dark:border-slate-700 shadow-sm active:bg-slate-50 dark:active:bg-slate-800 flex-row items-center">
      ...
      <View className="w-8 h-8 rounded-full bg-primary-50 dark:bg-primary-900 items-center justify-center"><Plus size={16} color="#2563eb" /></View>
    </TouchableOpacity>
  );
}
      ...
      <Modal visible={showCustomers} transparent animationType="slide" onRequestClose={() => setShowCustomers(false)}>
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white dark:bg-slate-900 rounded-t-3xl p-5 max-h-[70%]">
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => { setCustomer(item); setShowCustomers(false); }} className="flex-row items-center py-3 border-b border-slate-100 dark:border-slate-800">
                  <View className="w-9 h-9 rounded-full bg-primary-50 dark:bg-primary-900 items-center justify-center mr-3"><User size={16} color="#2563eb" /></View>
                  <View className="flex-1">
            ...
            />
            <TouchableOpacity onPress={() => { setShowCustomers(false); router.push('/(app)/customers/new'); }} className="bg-primary-50 dark:bg-primary-900 rounded-xl py-3 items-center mt-3 flex-row justify-center">
              <Plus size={16} color="#2563eb" />
              <Text className="text-primary-700 dark:text-primary-200 font-semibold text-sm ml-1">{t('customers.addCustomer')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <View className={`flex-1 ${isCompact ? 'flex-col' : 'flex-row'}`}>
        <View className="flex-1 p-4">
          <View className="flex-row items-center gap-2 mb-3">
            ...
            <TouchableOpacity onPress={listening ? stopListening : startListening} className={`rounded-lg px-3 py-2.5 ${listening ? 'bg-red-50 dark:bg-red-950' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700'}`}>
              ...
              {customer && (
                <View className="mt-3 flex-row items-center gap-2">
                  ...
                  <View className="flex-1">
                    ...
                    <View className={`rounded-lg px-3 py-2 ${balance > 0 ? 'bg-amber-50 dark:bg-amber-950' : 'bg-emerald-50 dark:bg-emerald-950'}`}>
                      <Text className={`text-sm font-semibold ${balance > 0 ? 'text-amber-700 dark:text-amber-300' : 'text-emerald-700 dark:text-emerald-300'}`}>{money(balance)}</Text>
                    </View>
                  </View>
                </View>
              )}
