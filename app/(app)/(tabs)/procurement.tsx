...
import { View, Text, FlatList, TouchableOpacity, ActivityIndicator, Modal, Share, Alert } from 'react-native';
import { useTranslation } from 'react-i18next';
import { useFocusEffect } from 'expo-router';
import { getLowStockProducts, type ProductWithCategory } from '../../../db/repositories/productRepository';
import {
...
function LowStockRow({ product, selected, onToggle }: { product: ProductWithCategory; selected: boolean; onToggle: () => void }) {
  ...
  return (
    <TouchableOpacity onPress={onToggle} className={`flex-row items-center py-3 px-4 border-b border-slate-100 dark:border-slate-800 ${selected ? 'bg-primary-50' : isOut ? 'bg-red-50' : 'bg-amber-50'}`}>
...
function POHistoryRow({ po, onReceive }: { po: PurchaseOrderWithVendor; onReceive: () => void }) {
  ...
  return (
    <View className="bg-white dark:bg-slate-900 mx-4 mb-3 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
      <View className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
        <View className="flex-row items-center justify-between">
          ...
          <View className={`rounded-full px-2.5 py-0.5 ml-2 ${isReceived ? 'bg-green-100' : 'bg-amber-100'}`}>
            <Text className={`text-xs font-semibold ${isReceived ? 'text-green-700' : 'text-amber-700'}`}>{po.status}</Text>
          </View>
        </View>
        {po.vendor_name ? (
      ...
      {!isReceived && (
        <TouchableOpacity onPress={onReceive} className="bg-primary-50 border-t border-primary-100 py-2.5 items-center active:bg-primary-100">
          <Text className="text-primary-700 font-semibold text-sm">✓ Mark as Received</Text>
        </TouchableOpacity>
      )}
    </View>
  );
      ...
      <View className="flex-1 bg-black/50 justify-end">
        <View className="bg-white dark:bg-slate-900 rounded-t-3xl p-6 max-h-[70%]">
          ...
          <FlatList
            ...
            keyExtractor={(v) => v.id}
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() => onSelect(item)} className="flex-row items-center py-3 border-b border-slate-100 dark:border-slate-800 active:bg-slate-50">
                <View className="flex-1">
        ...
        <View className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md">
          ...
          {vendor && (
            <View className="bg-primary-50 rounded-xl p-3 mb-4">
              <Text className="text-xs font-semibold text-primary-700 mb-0.5">Vendor: {vendor.name}</Text>
              {vendor.contact_person ? <Text className="text-xs text-primary-600">{vendor.contact_person}</Text> : null}
              <View className="flex-row gap-3 mt-1">
                {vendor.phone ? <Text className="text-xs text-primary-600">{vendor.phone}</Text> : null}
                {vendor.email ? <Text className="text-xs text-primary-600">{vendor.email}</Text> : null}
              </View>
            </View>
          )}
...
export default function ProcurementScreen() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const [stock, allOrders, allVendors] = await Promise.all([
      getLowStockProducts(),
      getPurchaseOrders(),
      getAllVendors(),
    ]);
  ...
  const proceedWithPO = async (vendor: VendorWithProductCount | null) => {
    ...
    const selectedProducts = lowStock.filter((p) => selected.has(p.id));
      ...
      <View className="px-4 pt-4 pb-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
        <View className="flex-row items-center justify-between mb-3">
          ...
          {tab === 'lowstock' && (
            ...
            >
              <Text className={`font-semibold text-sm ${selected.size > 0 ? 'text-white' : 'text-slate-400 dark:text-slate-500'}`}>{t('procurement.generatePO')}</Text>
            </TouchableOpacity>
          )}
        </View>
        {/* Tab switcher */}
        <View className="flex-row mb-0">
          {(['lowstock', 'orders'] as Tab[]).map((t_) => (
            ...
            >
              <Text className={`text-sm font-semibold ${tab === t_ ? 'text-primary-600' : 'text-slate-500 dark:text-slate-400'}`}>
