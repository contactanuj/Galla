...
function NodeRow({ node, depth, nodes, expanded, selectedId, onToggle, onSelect, t }: RowProps) {
  ...
  return (
    <View>
      <TouchableOpacity onPress={() => onSelect(node)} className={`flex-row items-center py-2.5 pr-3 border-b border-slate-100 dark:border-slate-800 ${selectedId === node.id ? 'bg-primary-50 dark:bg-primary-900' : 'bg-white dark:bg-slate-900'}`} style={{ paddingLeft: depth * 18 + 6 }}>
        {children.length > 0 ? (
            ...
            <ScrollView contentContainerClassName="p-4">
              ...
              {isAdmin && (
                <View className="flex-row flex-wrap gap-2 mb-4">
                  <TouchableOpacity onPress={() => openAdd(sel.id, nextType(sel.type))} className="flex-row items-center bg-primary-50 dark:bg-primary-900 rounded-lg px-2.5 py-2">
                    <Plus size={14} color="#2563eb" /><Text className="text-primary-700 dark:text-primary-200 text-xs font-medium ml-1">{t('layout.addChild')}</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity onPress={() => handleDelete(sel)} className="flex-row items-center bg-rose-50 dark:bg-rose-950 rounded-lg px-2.5 py-2">
                    <Trash2 size={14} color="#e11d48" /><Text className="text-rose-600 dark:text-rose-300 text-xs font-medium ml-1">{t('layout.delete')}</Text>
                  </TouchableOpacity>
                </View>
              )}
