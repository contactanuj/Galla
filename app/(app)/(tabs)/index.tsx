...
function QuickAction({ label, icon, onPress }: { label: string; icon: React.ReactNode; onPress: () => void }) {
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.8} className="bg-primary-50 rounded-xl p-4 flex-1 min-w-[140px] items-center active:bg-primary-100">
      {icon}
      <Text className="text-primary-700 font-semibold text-sm mt-2">{label}</Text>
    </TouchableOpacity>
  );
}
...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    ...
    async function loadStats() {
      ...
      setStats({ total: products.length, lowStock: lowStock.length, sales: todaySales, conflicts: conflicts.length });
    }
    loadStats();
  }, [isSysAdmin]);
        ...
        <>
          ...
          <View className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <View className="flex-row flex-wrap gap-3">
              <QuickAction label={t('pos.title')} icon={<ShoppingCart size={22} color="#1d4ed8" />} onPress={() => go('/pos')} />
              <QuickAction label={t('inventory.title')} icon={<Boxes size={22} color="#1d4ed8" />} onPress={() => go('/inventory')} />
              <QuickAction label={t('customers.title')} icon={<Users size={22} color="#1d4ed8" />} onPress={() => go('/(app)/customers')} />
              <QuickAction label={t('procurement.title')} icon={<ClipboardList size={22} color="#1d4ed8" />} onPress={() => go('/procurement')} />
            </View>
          </View>
        </>
      )}
