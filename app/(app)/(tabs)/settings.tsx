...
function MenuRow({ icon, label, onPress, color = '#475569' }: { icon: React.ReactNode; label: string; onPress: () => void; color?: string }) {
  return (
    <TouchableOpacity onPress={onPress} className="flex-row items-center px-4 py-3.5 active:bg-slate-50">
      {icon}
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50 dark:bg-slate-950">
      <View className="p-4">
        ...
        {isSystemAdmin && (
          <TouchableOpacity onPress={() => go('/(app)/admin')} className="bg-violet-50 rounded-xl py-3.5 items-center border border-violet-200 active:bg-violet-100 mb-4 flex-row justify-center">
            <ShieldCheck size={18} color="#7c3aed" />
            <Text className="text-violet-700 font-semibold text-base ml-2">{t('settings.adminPanel')}</Text>
          </TouchableOpacity>
        )}

        <TouchableOpacity onPress={logout} className="bg-rose-50 rounded-xl py-3.5 items-center border border-rose-200 active:bg-rose-100 flex-row justify-center">
          <LogOut size={18} color="#e11d48" />
          <Text className="text-rose-600 font-semibold text-base ml-2">{t('settings.logout')}</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
