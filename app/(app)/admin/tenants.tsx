  ...
  return (
    <View className="bg-white dark:bg-slate-900 mx-4 mb-3 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
      ...
      <View className="flex-row gap-2">
        ...
        <TouchableOpacity onPress={onToggle} className={`flex-1 rounded-lg py-2 items-center ${tenant.is_active ? 'bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-900' : 'bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-900'}`}>
          <Text className={`text-xs font-medium ${tenant.is_active ? 'text-amber-700 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>{tenant.is_active ? 'Deactivate' : 'Activate'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
