...
const ROLE_COLORS: Record<string, string> = {
  system_admin: 'bg-violet-100 text-violet-700 dark:text-violet-300',
      ...
      <View className="flex-row gap-2">
        {user.role !== 'system_admin' && (
          <>
            ...
            <TouchableOpacity onPress={onToggle} className={`flex-1 rounded-lg py-2 items-center ${user.is_active ? 'bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-900' : 'bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-900'}`}>
              <Text className={`text-xs font-medium ${user.is_active ? 'text-amber-700 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>{user.is_active ? 'Disable' : 'Enable'}</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
        ...
        if (supabaseEnabled) {
          ...
          if (authErr || !authData.user) {
            ...
            Alert.alert('Error', /already registered|already exists/i.test(msg)
              ? 'That email already has a login. If they cannot sign in, an earlier attempt created the account without a profile - recover it in Supabase (Authentication > Users > copy UID > add a user_profiles row), or use a different email.'
              : msg);
            setSaving(false);
            return;
          }
      ...
      <View className="px-4 pt-4 pb-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
        <View className="flex-row items-center mb-3">
          <TouchableOpacity onPress={() => router.back()} className="mr-3">
            <Text className="text-primary-600 dark:text-primary-300 font-medium">← Back</Text>
          </TouchableOpacity>
