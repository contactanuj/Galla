...
export default function NewProductScreen() {
  ...
  useEffect(() => {
    (async () => { setCategories(await getAllCategories()); setUnits(await getAllUnits()); })();
  }, []);
      ...
      <ScrollView className="flex-1" contentContainerClassName="p-4">
        <View className="items-center mb-5">
          ...
          <TouchableOpacity onPress={pickImage} className="flex-row items-center mt-2">
            ...
            <Text className="text-primary-600 text-xs font-medium ml-1">{image ? t('image.change') : t('image.add')}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <View className="mb-4">
            <CategoryTreePicker categories={categories} value={categoryId} onChange={setCategoryId} noneLabel={'Uncategorized'} />
          </View>
