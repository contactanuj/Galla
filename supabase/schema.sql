...
-- ============================================================
-- Customer payments (standalone credits / dues clearance).
-- A customer can pay money without a purchase, so dues are a ledger.
-- ============================================================
...
DO $$
BEGIN
  EXECUTE 'DROP POLICY IF EXISTS "tenant_read_customer_payments"   ON customer_payments';
  EXECUTE 'DROP POLICY IF EXISTS "tenant_insert_customer_payments" ON customer_payments';
  EXECUTE 'DROP POLICY IF EXISTS "tenant_update_customer_payments" ON customer_payments';
  EXECUTE 'DROP POLICY IF EXISTS "tenant_delete_customer_payments" ON customer_payments';
  EXECUTE 'CREATE POLICY "tenant_read_customer_payments"   ON customer_payments FOR SELECT USING (is_system_admin() OR tenant_id = get_my_tenant_id())';
  EXECUTE 'CREATE POLICY "tenant_insert_customer_payments" ON customer_payments FOR INSERT WITH CHECK (is_system_admin() OR tenant_id = get_my_tenant_id())';
  EXECUTE 'CREATE POLICY "tenant_update_customer_payments" ON customer_payments FOR UPDATE USING (is_system_admin() OR tenant_id = get_my_tenant_id())';
  EXECUTE 'CREATE POLICY "tenant_delete_customer_payments" ON customer_payments FOR DELETE USING (is_system_admin() OR tenant_id = get_my_tenant_id())';
END $$;

SELECT add_to_realtime('customer_payments');
